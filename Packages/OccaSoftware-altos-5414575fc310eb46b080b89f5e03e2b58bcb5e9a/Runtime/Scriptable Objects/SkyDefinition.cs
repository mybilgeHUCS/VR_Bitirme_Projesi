...
namespace OccaSoftware.Altos.Runtime
{
  ...
  public class SkyDefinition : ScriptableObject
  {
    ...
    /// <summary>
    /// The blend factor between the horizon and zenith.
    /// </summary>
    [Min(0)]
    public float skyColorBlend = 4f;
    ...
    public List<PeriodOfDay> periodsOfDay = new List<PeriodOfDay>()
    {
      new PeriodOfDay(
        "Night",
        6f,
        new Color32(36, 21, 36, 255),
        new Color32(35, 4, 46, 255),
    ...
    /// <summary>
    /// Returns a value in range [0,1] where 0 represents midnight and 1 represents noon.
    /// </summary>
    /// <returns></returns>
    ...
    private void HandleSkyColor(float time)
    {
      ...
      skyColors.equatorColor = Color.Lerp(
        periodsOfDay[currentPeriod].horizonColor,
        periodsOfDay[nextPeriodIndex].horizonColor,
        t
      );
      skyColors.skyColor = Color.Lerp(
        periodsOfDay[currentPeriod].zenithColor,
        periodsOfDay[nextPeriodIndex].zenithColor,
        t
      );
      skyColors.groundColor = Color.Lerp(
        periodsOfDay[currentPeriod].groundColor,
        periodsOfDay[nextPeriodIndex].groundColor,
        t
      );
    }
    ...
    public void SetLightingEnvironment(SkyColorSet skyColorSet)
    {
      ...
      exposure *= (
        1.0f
        - Mathf.Max(
          AltosSkyDirector.Instance.GetCurrentPrecipitation(),
          AltosSkyDirector.Instance.GetCurrentCloudiness()
        ) * 0.8f
      );
      ...
      RenderSettings.ambientSkyColor = Exposure(
        Saturation(skyColorSet.skyColor, environmentLightingSaturation),
        exposure
      );
    }
  }
